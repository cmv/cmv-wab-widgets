...
bdd.describe('Analysis toolValidate test', () => {
  ...
  bdd.describe('sub suite -- check extract data tool', function() {
    bdd.before(() => {
      this.privilegeUtil = {
        getUser: () => {
          return {
    ...
    bdd.it('FeatureLayer with url containing orgId but user is not admin should not support extract', () => {
      let layerObject: Analysis.LayerLike = {
        id: 'layer0',
        geometryType: 'esriGeometryPoint',
        declaredClass: 'esri.layers.FeatureLayer',
    ...
    bdd.it('FeatureLayer with url not containing orgId and user is admin should not support extract', () => {
      let layerObject: Analysis.LayerLike = {
        id: 'layer0',
        geometryType: 'esriGeometryPoint',
        declaredClass: 'esri.layers.FeatureLayer',
        url: 'http://www.arcgis.com/rest/services/xxx' // not contain user's orgId
      };
    ...
    bdd.it('FeatureLayer with url containing orgId and user is admin should support extract', () => {
      let layerObject: Analysis.LayerLike = {
        id: 'layer0',
        geometryType: 'esriGeometryPoint',
        declaredClass: 'esri.layers.FeatureLayer',
        url: 'http://www.arcgis.com/oC086ufSSQ6Avnw2/rest/services/xxx' // contain user's orgId
      };
