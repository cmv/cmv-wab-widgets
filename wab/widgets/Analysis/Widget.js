...
function(declare, lang, html, array, domStyle, domAttr, domClass, Deferred, on,
  ...
  portalUrlUtils, LayerInfos, exportUtils, ExportChooser, layerUtil, toolValidate, PrivilegeUtil,
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      if (this.currentToolSetting.dijitID.indexOf('ExtractData') < 0) {
        ...
        if (res.value.itemId) {
          ...
          if(this.currentToolSetting.allowToExport){
            this._createExportNode(featureLayer.title,
              new FeatureSet(res.value.featureSet),
              exportUtils.TYPE_FEATURESET);
          }
          domStyle.set(this.resultLoading, 'display', 'none');
        }
      }
        ...
        return esriRequest({
          ...
          array.forEach(tables, function(table) {
            ...
            def = LayerInfos.getInstance(this.map, this.map.itemInfo)
              ...
              if(this.currentToolSetting.allowToExport) {
                defs.push(this._createExportNodeForUrl(table.name, tableUrl, exportUtils.TYPE_TABLE));
              }
            }));
            defs.push(def);
          }, this);
          array.forEach(layers, function(layer) {
            ...
            if(this.currentToolSetting.allowToExport) {
              defs.push(this._createExportNodeForUrl(layer.name, layerUrl, exportUtils.TYPE_FEATURESET));
            }
            defs.push(def);
          }, this);
    ...
    _createExportNodeForUrl: function(label, url, type){
      var queryParams = new EsriQuery();
      queryParams.returnGeometry = (type === exportUtils.TYPE_FEATURESET);
      ...
      return queryTask.execute(queryParams).then(lang.hitch(this, function(featureSet) {
        ...
        if(type === exportUtils.TYPE_FEATURESET){
          this._createExportNode(label, featureSet, type, node);
        }else if(type === exportUtils.TYPE_TABLE){
          ...
          if(featureSet.fields){
            ...
            for(var p in featureSet.features[0].attributes){
              fields.push({
                name: p
              });
            }
          }
          this._createExportNode(label, {
            fields: fields,
            datas: data
          }, type, node);
        }
    ...
    _createExportNode: function(label, data, type, container){
      if(!container){
      ...
      var exportChooser = new ExportChooser({
        dataSource: ds
      });
      exportChooser.hide();
      html.place(exportChooser.domNode, this.domNode);
      ...
      this.own(on(exportNode, 'click', lang.hitch(this, function(event){
        ...
        exportChooser.show(event.clientX, event.clientY);
      })));
    },
