    ...
    */
    _displayBufferInputOptions: function () {
      ...
      if (this.config.bufferInputOption === "slider") {
        ...
        domClass.add(this.layerListOuterDiv, "esriCTOverrideListOuterContainer");
      }
    ...
    _setBufferSliderVisiblity: function () {
      ...
      if (this.config.searchLayers && this.config.searchLayers.length > 0) {
        ...
        if (this.config.intersectSearchedLocation && hideHorizontalSliderFlag) {
          ...
          //when both slider and textbox is configured, we override the height to reduce the space
          //since we are hiding entire bufferoption in polygon case remove the overridden style as 
          //it will result in removing space between search and layer/feature list
          if (this.config.bufferInputOption === "sliderAndTextbox") {
            domClass.remove(this.layerListOuterDiv, "esriCTOverrideListOuterContainer");
          }
        } else {
          domClass.remove(this.bufferOptionParentNode, "esriCTHidden");
          ...
          //in case of slider and textbox override the height to reduce the space between slider and list
          if (this.config.bufferInputOption === "sliderAndTextbox") {
            domClass.add(this.layerListOuterDiv, "esriCTOverrideListOuterContainer");
          }
        }
