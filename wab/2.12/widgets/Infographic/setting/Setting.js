...
define([
  ...
  function(on, query, Deferred, declare, BaseWidgetSetting, _WidgetsInTemplateMixin, html, lang, all,
    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      _preprocessingDataSourceForRange: function() {
        var mainDijitJson = this._getMainDijitJson();
        var type = mainDijitJson && mainDijitJson.type;
        if (type !== 'gauge') {
        ...
        var def1 = this.igUtils.preprocessingDataSourceForRange(rangeSources[0]);
        var def2 = this.igUtils.preprocessingDataSourceForRange(rangeSources[1]);

        return all([def1, def2]).then(function(res) {
          var ret = res[0];
          if (ret) {
            this.rangeDefinition1 = ret.definition;
            this.range1Features = ret.features;
          }
          var ret1 = res[1];
          if (ret1) {
            this.rangeDefinition2 = ret1.definition;
            this.range2Features = ret1.features;
          }
          return;
        }.bind(this));
      },
      ...
      _preprocessingDataSource: function(dataSource) {
        ...
        var def2 = this._preprocessingDataSourceForRange();
