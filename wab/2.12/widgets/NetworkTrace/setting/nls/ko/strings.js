define({
  ...
  "projectSetting": {
    ...
    "selectLabel": "선택",
    "polygonLayerHelp": "<p>다음과 같은 조건의 폴리곤 레이어가 표시됩니다.<br/><li>'생성', '삭제', '업데이트'라는 편집 기능이 있어야 하는 레이어</li><li>다음과 같은 이름과 데이터 유형이 사용된 2개의 필드가 있어야 하는 레이어</li><ul><li>name(문자열 유형 필드)</li><li>globalid(GlobalID 유형 필드)</li></ul><p/>",
    "outputParameterHelp": "<p>작업 URL의 결과 폴리곤 레이어가 표시됩니다.<p/>",
    "pointLayerHelp": "<p>다음과 같은 조건의 포인트 레이어가 표시됩니다.<br/><li>'생성', '삭제', '업데이트'라는 편집 기능이 있어야 하는 레이어</li><li>다음과 같은 이름과 데이터 유형이 사용된 2개의 필드가 있어야 하는 레이어</li><ul><li>inputtype(문자열 유형 필드)</li><li>projectid(GUID 유형 필드)</li></ul><p/>"
  },
  "inputOutputTab": {
    "title": "입력/결과",
    ...
    "saveToLayerHelp": "<p>다음과 같은 조건의 레이어가 표시됩니다.<br/><li>'생성', '삭제', '업데이트'라는 편집 기능이 있어야 하는 레이어</li><li>다음과 같은 이름과 데이터 유형이 사용된 2개의 필드가 있어야 하는 레이어</li><ul><li>parametername(문자열 유형 필드)</li><li>projectid(GUID 유형 필드)</li></ul><p/>"
  },
  "summaryTab": {
    "title": "요약",
  ...
  "generalSettings": {
    "title": "일반 설정",
    "displayTextForButtonLegend": "'실행' 버튼의 표시 텍스트",
    "displayTextforButtonDefaultValue": "실행",
    "displayTextForButton": "표시 텍스트",
    "autoZoomAfterTrace": "확대 옵션",
    "zoomText": "추적 후 자동 확대"
  },
