    ...
    if(rendererName === 'DrawResultFeatureSet'){
      ...
      array.forEach(values, function (val) {
        ...
        if (['GPLong', 'GPDouble', 'GPString', 'GPBoolean']
          .some(function (s) { return param.dataType.includes(s); })) {
          text += utils.sanitizeHTML(val);
        } else if (param.dataType.includes('GPLinearUnit')) {
          text += val.distance + '&nbsp;' + val.units;
        } else if (param.dataType.includes('GPDate')) {
          text += new Date(val).toLocaleTimeString();
        } else if (param.dataType.includes('GPRecordSet')) {
          text += 'table';
        } else if (param.dataType.includes('GPDataFile') || param.dataType.includes('GPRasterDataLayer')) {
          if (val.url) {
