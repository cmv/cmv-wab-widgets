    ...
    _generateValueFormatter: function(valueDisplay) {
      ...
      return lang.hitch(this, function(valueDisplay, val) {
        ...
        if (valueDisplay.isRatio) {
          if (val <= min) {
            value = jimuUtils.convertNumberToPercentage(0, decimalPlaces);
          } else {
            var base = max - min;
            var v = val - min;
            value = jimuUtils.convertNumberToPercentage(v / base, decimalPlaces);
          }
        } else { //unit
        ...
        if (!valueDisplay.isRatio && dataFormat.unit !== 'percentage') {
          ...
          value = jimuUtils.localizeNumber(value);
        }
